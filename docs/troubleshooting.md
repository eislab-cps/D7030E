...
| Multi-hop throughput zero beyond one hop | Ad-hoc multi-hop needs a routing protocol that has converged. The Lab 03 chain starters enable OLSR (unless you pass `--routing=aodv`) and wait 30 simulated seconds before sending | Keep both. If a chain still reports zero with the arguments your lab asks for, record the command and report it to a supervisor — do not change the scenario to make a number appear |
